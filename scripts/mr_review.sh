...
# Check if project context file exists (from project understanding analysis)
PROJECT_CONTEXT=""
if [ -n "${PROJECT_CONTEXT_FILE:-}" ] && [ -f "${PROJECT_CONTEXT_FILE}" ]; then
  echo "[INFO] Loading project context from ${PROJECT_CONTEXT_FILE}"
...
if [ -n "${PROJECT_CONTEXT}" ]; then
  REVIEW_PROMPT="## Project Context (Generated by AI Analysis)

The following project understanding context was generated by multi-agent analysis to help you better understand the codebase:
...
${REVIEW_PROMPT}"
  echo "[INFO] Project context added to review prompt"
fi
