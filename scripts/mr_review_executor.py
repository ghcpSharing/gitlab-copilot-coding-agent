...
{prompt}"""
    ...
    try:
        ...
        # 保存 stderr（如果有）
        if result.stderr:
            (subtask_workspace / "stderr.txt").write_text(result.stderr, encoding='utf-8')
        
        if result.returncode != 0:
            print(f"[WARN] Copilot returned non-zero exit code: {result.returncode}")
            print(f"[WARN] stderr: {result.stderr[:500]}")
            print(f"[WARN] Stderr saved to {subtask_workspace / 'stderr.txt'}")
...
Duration: {subtask.estimated_time_seconds}s
Files: {', '.join(subtask.files)}

Command: copilot --allow-all-tools --allow-all-paths
Working Directory: {subtask_workspace}
...
"""
        (subtask_workspace / "timeout_error.log").write_text(error_log, encoding='utf-8')
        print(f"[INFO] Timeout details saved to {subtask_workspace / 'timeout_error.log'}")
        
        return {
            'status': 'timeout',
            'error': f'Timeout after {subtask.estimated_time_seconds}s (see timeout_error.log)',
            'findings': []
        }
...
Working Directory: {subtask_workspace}
"""
        (subtask_workspace / "error.log").write_text(error_log, encoding='utf-8')
        print(f"[INFO] Error details saved to {subtask_workspace / 'error.log'}")
