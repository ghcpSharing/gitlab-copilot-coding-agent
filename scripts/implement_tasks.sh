...
if [[ -f copilot.patch ]]; then
  echo "[INFO] Attempting to apply patch..."
  if git apply copilot.patch 2>/dev/null; then
    echo "[INFO] Patch applied successfully"
    PATCH_APPLIED=true
  else
    echo "[WARN] Standard patch application failed, attempting with --reject..."
    if git apply --reject copilot.patch 2>/dev/null; then
      echo "[INFO] Patch applied with rejects"
      PATCH_APPLIED=true
    else
      echo "[WARN] Patch could not be applied, checking for direct file generation..."
    fi
  fi
else
  echo "[INFO] No patch file generated by Copilot"
fi
...
UNTRACKED_FILES=$(git ls-files --others --exclude-standard | grep -v -E '(patch_raw\.txt|copilot\.patch|todo\.md|todo_completed\.md|__pycache__|\.pyc$)')
